...
class ImageToArrayConverter(object):
    ...
    def __init__(self, src):
        ...
        #bw = cv2.GaussianBlur(bw, (7,7), 0) # When the digits are really small, this line tends to join digits together or mess them up
        ret, thbw = cv2.threshold(bw, 200, 255, cv2.THRESH_BINARY_INV)
        thbw = cv2.erode(thbw, np.ones((1,1), np.uint8), iterations = 2)
        image, cntrs, hier = cv2.findContours(thbw.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # Using RETR_XTERNAL eliminates the error of capturing curves in a didgit as two digits
